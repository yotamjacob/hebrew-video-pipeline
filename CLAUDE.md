...
**Email layer (Resend)** — accounts collect an email at registration (`_EMAIL_RE`) and get a verification email; verification is a **nudge, not a gate** (`email_verified` on the record; `/auth/me` returns `email` + `email_verified`; the site shows a dismissible banner). Routes: `/auth/verify` (public GET, HTML page), `/auth/request-verification` (authed POST, also sets/updates the address), `/auth/forgot` (public POST, always 200 — never reveals account existence), `/auth/reset` (public POST). Reset/verify links carry a scoped one-time token (`_sign_scoped_token`/`_verify_scoped_token`, scope `verify`|`reset` baked into the HMAC message so they're not interchangeable; verify TTL 7d, reset TTL 1h). `_send_email` posts to Resend and is **best-effort** — no `RESEND_API_KEY` just logs and skips, so sign-up never fails on mail. Secret keys needed in `hebpipe-auth`: `RESEND_API_KEY`, `EMAIL_FROM` (verified-domain sender), optional `SITE_URL` (reset links; defaults to the Vercel alias). `email:{addr}` reverse index in `hebpipe-users` powers forgot-by-email. Registration has **two mandatory checkboxes**, both enforced client-side (`authSubmit` → `auth.requiredError`) and, for terms, server-side (`/auth/register` 400s without `terms_accepted`), both timestamped on the user record: **Terms/Privacy acceptance** (`terms_accepted` → `terms_accepted_ts`) and **consent to updates + promotions** (`auth.marketing` checkbox → `marketing_consent: true` + `marketing_consent_ts`). `/auth/me` returns `marketing_consent`; `site/legal.html` documents that update+promotional email goes to registered users per the sign-up agreement. ⚠️ COMPLIANCE NOTE: the marketing box text is "אני מאשר/ת קבלת עדכונים ופרסומים מפייפליין" (updates **and promotions**) AND it is mandatory — this was a deliberate owner decision made after being warned that mandatory consent to promotions violates Israeli Communications Law Amdt. 40 / GDPR (advertising consent must be a freely-given opt-in, not a condition of signup). If revisited, the compliant fix is to make this box optional (unchecked) or split promotions into a separate optional opt-in.
