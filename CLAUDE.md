...
| `site/app.css` | All frontend styles. **Design system (2026-07-05 "Tel Aviv indie studio" identity):** warm paper `--bg:#F6F2EA` + subtle SVG grain on `body`, NO gradients/glow; brand = softened grape (`--purple-*` vars redefined, primary `#6B5CA5`), calm contrast = sage (`--green/--sage #6F9E7E`, used for done states + hero accents), warm ink text `#2B2536`. Fonts: Rubik (Hebrew wordmark/headings), Assistant (Hebrew body), Space Grotesk + Space Mono (Latin/mono accents via `.mono`, `.check-time`, etc.). Section-header/tab emojis were stripped from i18n strings (words kept); `.option-icon`/`.upload-icon` are crafted inline-SVG line icons (grape, 1.7 stroke). Checklist has a dashed pipeline-flow connector. |
