...
(function () {
  ...
  const I18N = {
    ...
    'guide.account.body':  { en: '<ul><li>Each account has a limited number of free video credits. One credit is used every time you process a video (the Options step).</li><li>Your remaining credits show in the pill at the top of the app.</li><li>Manage your email and verification from the banner and your account.</li><li>Need more credits or help? Use the Contact link in the footer.</li></ul>', he: '<ul><li>לכל חשבון יש מספר מוגבל של קרדיטים חינמיים לסרטונים. קרדיט אחד מנוצל בכל עיבוד סרטון (שלב האפשרויות).</li><li>הקרדיטים שנותרו מוצגים בתווית שבראש האפליקציה.</li><li>נהלו את האימייל והאימות מהבאנר ומהחשבון.</li><li>צריכים עוד קרדיטים או עזרה? השתמשו בקישור "צרו קשר" בתחתית.</li></ul>' },
