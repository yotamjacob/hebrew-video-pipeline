...
| Piece | Where | Contract |
|---|---|---|
| Button + editor | `site/assembler.html` `socialBlock(c)` (`.o-social` in each candidate card's `.c-out`) | Posts `{upload_key, start, end, video_key, title, hook}` - the clip's CURRENT range (after the trim steppers) and the rendered clip's key once its render landed (else `""`, frames are optional). Result: editable `textarea.o-cap` (caption) and `.o-copy`, which copies the caption. No hashtags (user 2026-09-24: "no need for hashtag generation anywhere" - the `input.o-tags` line was removed). The text is stored on the candidate (`c.social`, next to `hookText`), so a re-render - which rebuilds the tiles - keeps it. Soft errors: `no_transcript` (the `_asm_words.json` scratch expired, 48 h) and `no_words` get specific Hebrew messages. |
...
| Prompt + post-step | `content_fns._assembler_social_prompt`, `_clean_social` (pure) | 2-3 Hebrew lines, GENDER-NEUTRAL address (plural imperative / infinitive), NO hashtags at all, <= 2 emojis; title + hook are context only ("do NOT copy them"). `_clean_social` does not trust the prompt: em / en dashes -> "-", every `#tag` stripped (`_strip_hashtags`), <= 3 caption lines -> `{caption}`. Spend logged as `assembler_social`. The main editor's default flavor (`post_caption` spend, `{caption}` shape) no longer asks for hashtags either and its output also passes `_strip_hashtags` (2026-09-24) - tested. |
...
| Karaoke RTL (review fix) | shared `build_caption_ass` (pipeline_fns) | User review: "rtl is not working correctly on the karaoke". Root cause (measured, local libass 0.17.5 AND the burn image's 0.17.1): libass bidi-reorders each override-tag-separated run on its own unless the style's `Encoding` is -1, so the highlight's color tags laid a Hebrew line's words out LEFT to right (and flipped a trailing comma to the line start). First fixed as an assembler-only post-step, then (user go, same day) moved INTO the shared builder: karaoke styles emit `Encoding -1`, classic / word keep `1` (byte-identical) - the main pipeline's karaoke burns and the exact-frame preview are fixed too. See docs/captions.md. |
...
| Previews, thumbnails, branding (follow-up #2, same day) | (1) No preview button: the moment a clip's render is done its card shows the player by itself (`preload=metadata`, one plays at a time). (2) `render_story` returns `thumb` = a 240 px frame of the FINISHED clip (~1 s into the body, past an intro; `_thumb_b64`, best-effort `""`) - the card image and the player poster switch to it (`c.lastThumb` keeps it while a refresh runs); the analysis thumbnail was the raw landscape source. (3) Branding changed after clips exist (intro / outro added or removed, logo added / removed / dragged / resized / opacity, fade toggle / length) -> `brandingChanged()` re-creates the picked clips automatically so the previews show the real branded result: debounced 1.5 s (a drag or slider settles first), coalesced (a change during a running batch queues ONE more). (4) The layout tiles and the logo stage are ABSTRACT (schematic speaker illustrations; the stage = an SVG silhouette + caption bar on the brand gradient) - never a frame of the user's video. |
...
Tests: `tests/frontend/assembler_setup.spec.js` (layout default + previews painted, no analysis before the click, the button disabled until the upload landed, refresh with a new layout, remembered options, the `uploaded` restore, the wrapped outro name, the viral card); every assembler spec now clicks "עיבוד" and expects the automatic first batch.
